...
def load_threshold(symbol):
    path = f'best_params_{symbol}.json'
...
def run_single_backtests():
    parser = argparse.ArgumentParser(description="Backtest Single Symbol")
    parser.add_argument('--symbol', type=str, default=None, help='Symbol to backtest')
    ...
    if args.symbol:
        ...
        symbols = ['EURUSD', 'USDJPY', 'XAUUSD', 'GBPUSD', 'BTCUSD']
    ...
    for sym in symbols:
        ...
        print(f"{sym} Sharpe Ratio: {sharpe:.2f}")
        print(f"{sym} Max Drawdown: {max_drawdown:.2f}%")
